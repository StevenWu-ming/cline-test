...
BOSS_REGION = (374, 317, 1300, 63)    # 圖像偵測區域
BOSS_THRESHOLD = 0.45                 # 相似度門檻
STEALTH_ICON_PATH = "stealth_buff.png"  # ← 替換為你的隱身圖示檔名
STEALTH_REGION = (1746, 34, 27, 26)    # ← 根據實際畫面調整區域
MAX_CHECKS = 6                        # 每次最多偵測幾次
CHANNEL_REGION = (797, 136, 33, 19)   # 頻道 OCR 區域
...
def change_channel():
    ensure_stealth()  # ✅ 換頻前施放隱身（如果沒 Buff）
...
def run_actions(max_count=2):
    ...
    try:
        while action_count < max_count:
            ...
            print(f"▶️ 開始第 {action_count+1} 次下跳")
            do_action()
            action_count += 1
...
def run_cycle():
    while True:
        change_channel()
        if wait_for_image("enter_ready1.png"):
            enter_game()
            run_actions(2)
        else:
            print("❌ 跳過進入遊戲")
        ...
        if detect_boss():
            print("🔔 發現 BOSS，立即進入自由市場")
            human_click(1693, 516)
            time.sleep(5)  # ⏳ 等畫面載入完成
